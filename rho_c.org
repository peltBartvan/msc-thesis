...
Preparing a sample of area $A$ and assuming that the current is uniformly distributed along the contact, the definition easily translates to $\rho_c = \frac{V}{I}A$, where the total current $I$ and induced voltage $V$ can easily be measured.
In reality, the current distribution into a contact can be localized near the edge of the contact, an effect called current crowding.
The current density decays exponentially with characteristic length $L_t = \sqrt{\frac{\rho_c}{R_\square}}$, called the transfer length, this phenomenon is illustrated in Figure \ref{fig:LtIllustration}.
Here $R_\square$ is the sheet resistance of the wafer.
...
This is the main challenge of contact resistivity measurements, reliably distilling local $J(V)$ behaviour from globally measured $I(V)$ data.

# The main difficulty lies in measuring the /local/ current density, $J$, when in practice it is typically a total current, $I$, that is measured.
# This challenge of trying to probe local current densities with only global measures such as total supplied current and some voltage needs to be addressed when performing contact resistivity measurements.

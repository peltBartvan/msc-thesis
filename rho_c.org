...
Several important process parameters are studied, such as \AZO{} doping, the impact of an annealing step, and the silicon doping.
...
Compared to conventional methods, a significant advantage is that it works on samples without any testing structures.
...
Moreover, a high degree of passivation should be achieved so that the generated free charge carriers do not recombine. \question{hier had je een punt staan, inmiddels heb ik "passivation" tot volzin gemaakt. Vind je het nu wel beter volgen?}.
...
\todo{wat bepaalt mobiliteit, beschrijven!}
...
Immediately clear is that doping the ZnO lowers the bulk resistivity by an order of magnitude.
This decreasing resistivity can be attributed to an increasing carrier density, as the Hall mobility decreases with doping.
...
Here, a clear difference is found between the capped and uncapped samples, as uncapped samples become more resistive by several orders of magnitude.
This shows that the presence of a capping layer is crucial for achieving good film conductivity.
...
This decrease in Hall mobility could be caused by increased impurity scattering on Al dopants.
...
As shown in Figure \ref{fig:omegasJsc}, annealing has a more pronounced effect on the scattering frequency.
